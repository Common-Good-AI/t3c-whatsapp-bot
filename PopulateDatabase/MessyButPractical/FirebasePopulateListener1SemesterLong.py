...
def initialize_event_collection(event_id, event_name, event_location, event_background, event_date, extraction_settings, languages, initial_message, completion_message):
    """
    Initializes the Firestore collection and stores event info, bot settings, 
    and survey questions within the 'info' document.
    """
    collection_ref = db.collection(f'AOI_{event_id}')
    info_doc_ref = collection_ref.document('info')
    
    # Define some example extra questions; you can add more or remove as needed
    extra_questions = {
        "ExtraQuestion1": {
            "enabled": True,
            "text": "Whats your school name?"
        },
        "ExtraQuestion2": {
            "enabled": False,
            "text": "whats your nickname?"
        },
        "ExtraQuestion3": {
            "enabled": True,
            "text": "whatsyour studentID "
        },
        "ExtraQuestion4": {
            "enabled": True,
            "text": "Do you have any specific requests for the organizers?"
        }
    }
    ...
    info_doc_ref.set({
        ...
        'extraction_settings': extraction_settings,
        'languages': languages,
        'extra_questions': extra_questions  # <--- Added
    })
